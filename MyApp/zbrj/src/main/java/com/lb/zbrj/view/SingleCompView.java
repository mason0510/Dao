...
import com.lb.zbrj.adapter.SingleCompAdapter;
...
public class SingleCompView extends VideoListView{
	
	private CompData data;

	public SingleCompView(Context context) {
		super(context);
	}
	
	public SingleCompView(Context context, AttributeSet attrs) {
		super(context, attrs);
	}
	
	@SuppressLint("NewApi")
	public SingleCompView(Context context, AttributeSet attrs, int defStyle) {
	...
	protected void setListAdapter(List<Object> videoList){
		...
		if (null == mWatchListAdapter) {
			mWatchListAdapter = new SingleCompAdapter(getContext(), videoList);
			((SingleCompAdapter)mWatchListAdapter).setComp(data);
			mPullToRefreshListView.setAdapter(mWatchListAdapter);
		} else {
			((SingleCompAdapter)mWatchListAdapter).setVideoList(mVideoList);
	...
	protected void getVideoList() {
		NetInterfaceStatusDataStruct niStatusData = new NetIF_ZBRJ(getContext()).m1_get_CompDetailvideoList(data.compid
				, mPageNum == 1 ? 0 : getLastVideoID()
				, Constants.PAGE_SIZE_INT, filterValue);
