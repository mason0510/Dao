...
public class CollectListAdapter extends BaseAdapter {
	...
	private CollectVideoHelper mCollectVideoHelper;
	private NetIF_ZBRJ netIf_ZBRJ = null;
	public AfterClearAllListener mAfterClearAllListener;
	public CollectListAdapter(Context context, List<Object> videoList) {
		this.mContext = context;
		mVideoList = videoList;
		layoutInflater = LayoutInflater.from(context);
		mCollectVideoHelper = new CollectVideoHelper(AccountData.getInstance().getBindphonenumber());
		...
		if(mVideoList.get(position) instanceof VideoData){
			...
			holder.del.setOnClickListener(new OnClickListener(){
				...
				public void onClick(View arg0) {
					mCollectVideoHelper.del(videoData.videoID);
