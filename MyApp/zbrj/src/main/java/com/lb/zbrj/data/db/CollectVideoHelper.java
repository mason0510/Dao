...
public class CollectVideoHelper {
	private SQLiteDatabase db;

	public CollectVideoHelper(String dbName) {
	...
	public ArrayList<VideoData> findAll() {
		...
		try{
			String sql = "select * from collect_video order by watchtime desc";
	...
	public ArrayList<VideoData> find(int start, int count) {
		...
		ArrayList<VideoData> datas = new ArrayList<VideoData>();
		try{
			String sql = "select * from collect_video order by watchtime desc limit " + start + "," + count;
			c = db.rawQuery(sql, null);
			if (c == null) {
				return null;
			}
	...
	public synchronized void del(String videoid) {
		try{
			db.delete("collect_video", "videoid=?", new String[]{videoid});
		}catch(Exception e){
			Log.e(Constants.LOG_TAG, e.getMessage(), e);
		}
	}
	public VideoData findOne(String videoid){
		Cursor c = null;
		VideoData data = null;
		try{
			String sql = "select * from collect_video where videoid=?";
			c = db.rawQuery(sql, new String[]{videoid});
			if (c == null) {
				return null;
			}
			if (c != null) {
	...
	public synchronized void delAll() {
		try{
			db.delete("collect_video", null, null);
		}catch(Exception e){
			Log.e(Constants.LOG_TAG, e.getMessage(), e);
		}
	}
	...
	public synchronized void add(VideoData data) {
		try{
			...
			db.insert("collect_video", null, cv);
		}catch(Exception e){
			Log.e(Constants.LOG_TAG, e.getMessage(), e);
		}
	}
	...
	public synchronized void initTestData(){
		//FIXME 测试数据
		db.execSQL("delete from collect_video");
		db.execSQL("insert into collect_video(videoid, nick, imageurl, title, watchtime) values('01','白玫瑰'" +
						",'http://c.hiphotos.baidu.com/image/pic/item/7c1ed21b0ef41bd5fff25ceb53da81cb39db3d0f.jpg'" +
						",'教你做好吃的手撕鸡', '2015-02-04 14:18:14')");
		db.execSQL("insert into collect_video(videoid, nick, imageurl, title, watchtime) values('02','美美丫头'" +
						",'http://c.hiphotos.baidu.com/image/pic/item/d6ca7bcb0a46f21f0491e3cef4246b600d33ae82.jpg'" +
						",'午后阳光', '2015-02-03 14:18:14')");
		db.execSQL("insert into collect_video(videoid, nick, imageurl, title, watchtime) values('03','捕风'" +
