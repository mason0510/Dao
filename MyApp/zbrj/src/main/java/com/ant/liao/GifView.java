    			...
    			if(pause == false){
	    			//if(gifDecoder.parseOk()){
	    				...
	    				if(redrawHandler != null){
	    					Message msg = redrawHandler.obtainMessage();
	    					redrawHandler.sendMessage(msg);
//	    					Thread.sleep(sp ==0?100:sp);
	    					SystemClock.sleep(sp ==0?100:sp); 
	    				}else{
	    					break;
	    				}
//	    			}else{
...
//	    			}
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		case MotionEvent.ACTION_UP:
			...
			// {
			// int dis = getTop();
			// this.layout(this.getLeft(), 0, this.getRight(), 0 +
			// this.getHeight());
			// disY = dis - getTop();
			// }
			...
			// this.layout(this.getLeft(), screenH-getHeight(), this.getRight(),
			// screenH);
			// }
			// }
			// if(getWidth()<=screenW)
