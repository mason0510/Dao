...
public class IMRecordSThreadsSearchActivity extends BaseActivity implements
		...
		if (!StringUtils.isNull(real_word)) {
			...
			if (mAdapter == null) {
				mAdapter = new IMRecordThreadsAdapter(
						IMRecordSThreadsSearchActivity.this, list);
			}
			...
			pool.submit(new Runnable() {
				...
				public void run() {
					try {
						...
						tempIdx.addAll(ImData.getInstance().getSIndexs());
						if(tempIdx.size() > 0){
						...
						IMRecordSThreadsSearchActivity.this
