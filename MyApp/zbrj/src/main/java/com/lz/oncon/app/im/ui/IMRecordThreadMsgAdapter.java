...
public class IMRecordThreadMsgAdapter extends BaseAdapter {
	...
	public IMRecordThreadMsgAdapter(Context context, IMThreadData data) {
	...
	@Override
	public int getCount() {
		return mData.getMsgs() == null ? 0 : mData.getMsgs().size();
	}

	@Override
	public Object getItem(int position) {
		return mData.getMsgs() == null ? null : mData.getMsgs().get(position);
	}
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		yRefreshView(holder, mData, position);
		return convertView;
	}
	...
	private void yRefreshView(ViewHolder holder, IMThreadData threadData, int position){
		if (threadData != null) {
			...
			SIXmppMessage theLastMessage = threadData.getMsgs().get(position);
