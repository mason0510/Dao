...
public class CompareIMSThread implements Comparator<String> {
	...
	public int compare(String id0, String id1) {
		try{
			...
			IMThreadData thread0 = ImData.getInstance().getSDatas().get(id0);
			if(thread0 == null)return 1;
			IMThreadData thread1 = ImData.getInstance().getSDatas().get(id1);
