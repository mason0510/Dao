    ...
    private static class RLDanmakusRetainer implements IDanmakusRetainer {
        ...
        public void fix(BaseDanmaku drawItem, IDisplayer disp) {
            if (drawItem.isOutside())
                return;
            if(!(drawItem instanceof R2LDanmaku))
            	return;
            float topPos = 0;
            ...
            if (!shown) {
                ...
                R2LDanmaku insertItem = null, firstItem = null, lastItem = null, minRightRow = null;
                boolean overwriteInsert = false;
                while (!mCancelFixingFlag && it.hasNext()) {
                    BaseDanmaku ii = it.next();
                    if(!(ii instanceof R2LDanmaku))
                    	continue;
                    R2LDanmaku item = (R2LDanmaku)ii;
                    if(item == drawItem){
                        insertItem = item;
                        lastItem = null;
                        shown = true;
                ...
                if (insertItem != null) {
                    if (lastItem != null)
                        topPos = lastItem.getBottom()-lastItem.initY;
                    else
                        topPos = insertItem.getTop()-insertItem.initY;
                    ...
                    if (minRightRow != null) {
                        topPos = minRightRow.getTop()-minRightRow.initY;
                    ...
                    topPos = firstItem.getTop()-firstItem.initY;
                    mVisibleDanmakus.removeItem(firstItem);
                    shown = false;
                } else {
                    topPos = 0;
                }
                topPos = checkVerticalEdge(overwriteInsert, (R2LDanmaku)drawItem, disp, topPos, firstItem,
                        lastItem);
        ...
        //fixme 修改下碰撞
        protected float checkVerticalEdge(boolean overwriteInsert, R2LDanmaku drawItem,
                IDisplayer disp, float topPos, R2LDanmaku firstItem, R2LDanmaku lastItem) {
        				
            if (topPos < 0 || (firstItem!=null && firstItem.getTop()-firstItem.initY > 0) || topPos + drawItem.paintHeight+drawItem.initY > disp.getHeight()) {
                topPos = 0;
                clear();
            }
            return topPos;
